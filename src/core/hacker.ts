...
const Css = {
  ...
  template: `#workbench\u005C\u002Eparts\u005C\u002Etitlebar::after{
      content: '';
      position: absolute;
      width: 100%;
      height: 125%;
...
class Hacker {
  ...
  private async inject(cssPath: string): Promise<void> {
    const colors = Css.colors.split(',');
    const template = Css.template.replace(/\n[\s]+/g, '');
    const style = colors.map((c) => template.replaceAll('{{color}}', c));
